<template>
	<view class="flex_container">
		...
		<view class="search_box flex">
			<uv-search class="flex-1" placeholder="请扫描或输入申请单" shape="square" :showAction="false" color="#ffffff"
				bgColor="#2D7EE7" placeholderColor="rgba(255, 255, 255, 0.45)"
				searchIconColor="rgba(255, 255, 255, 0.45)" height="66rpx" borderColor="rgba(255, 255, 255, 0.12)"
				v-model="scanCode" @search="scanConfirm">
			</uv-search>
		...
		<view class="flex-1 container_box flex-y">
			...
			<template v-else>
				...
				<scroll-view class="check_container flex-1" scroll-y>
					<view class="check_item flex" v-for="el,index in nowInfo.list" :key="index"
						...
						<view class="flex-1">
							<view class="flex">
								...
								<view class="flex" @click.stop="el.isShow=!el.isShow">
									...
									<uv-icon :name="el.isShow?'arrow-up-fill':'arrow-down-fill'" :size="20"
										color="rgba(0, 0, 0, 0.45)"></uv-icon>
								</view>
							</view>
							<view class="simple_box flex" v-if="!el.isShow">
								...
								<view @click.stop="">
									<view class="center_btn" :class="{align:el.againConfirm==1}"
										@click="singleConfirm(index)">{{el.againConfirm==1?'再次确认':'确认'}}</view>
								</view>
							</view>
							<template v-else>
								<view class="flex flex-wrap" style="margin-top: 8rpx;">
									...
									<view class="detail_item">
										...
										<view>{{el.rpt_dept|defaultVal}}</view>
									</view>
									<view class="detail_item">
		...
		<view class="select_box padding-safe-area flex flex-between" v-if="!isNone">
			...
			<uv-button shape="circle" type="primary" @click="confirmAll">确认勾选选项</uv-button>
		</view>
		<view class="refresh_box" @click="refreshInfo">
			<uv-icon name="reload" color="#fff" :size="34"></uv-icon>
		</view>
		<uv-action-sheet ref="actionSheet" :actions="[{name:'扫描'},{name:'输入'}]" title="请扫描患者腕带确认" closeOnClickOverlay
			cancelText="取消" :closeOnClickAction="false" @select="selectSheet" @close="isFirst=true">
		</uv-action-sheet>
	</view>
</template>
<script>
		...
		methods: {
			scanConfirm() {
				if (this.isFirst) {
					...
					if (this.singleIndex || this.singleIndex === 0) {
						...
						// params = {patientRecordsList:[{rpt_no:data.rpt_no,rpc_code:this.deptCode,type:data.type}],code:this.deptCode,wristbandCode:this.handCode};
						params = {
							patientRecordsList: [{
								...data,
						...
						params = {
							patientRecordsList: this.nowInfo.list.filter(el => el.isCheck).map(el => {
								return {
									rpc_code: this.deptCode,
									...el
								}
			...
			confirmAll() {
				...
				if (list.length == 0) {
					...
					// 	}),code:this.deptCode}
					let params = {
						patientRecordsList: list.map(el => {
							return {
								rpc_code: this.deptCode,
								...el
							}
					...
					if (this.nowInfo.type != 3) {
						...
						// 						that.confirmApi({...params,wristbandCode:res.content});
						// 					}
						// 				}
						// 			})
						// 		}else{
