      ...
      <div>
        ...
        >
          <el-form-item label="工号">
            <el-input
              v-model="searchFormData.useAccount"
              placeholder="请输入工号"
              clearable
          ...
          <el-form-item label="病区/科室">
            <el-input
              v-model="searchFormData.regionName"
              placeholder="请输入病区/科室"
              clearable
              @keyup.enter.native="queryData"
            ></el-input>
          </el-form-item>
          <el-form-item label="状态">
            <el-select
              v-model="searchFormData.userStatus"
              placeholder="全部"
              clearable
            >
              <el-option
      ...
      <div>
        ...
        >
          ...
          >
            <!-- <template slot-scope="scope">
              {{
                scope.row.regionList
                  .filter((x) => scope.row.regionValue.includes(x.regionValue))
                  .map((item) => {
                    return;
                    item.regionName;
                  })
          ...
          <el-table-column label="操作" align="center" width="200">
            <template slot-scope="scope">
              <el-button @click="editMethods(scope.row, $event)" type="text"
                >编辑</el-button
              >
              <el-button
                @click="resetMethods(scope.row, $event)"
                type="text"
                style="color: #e6a23c"
                >重置
              </el-button>
              <el-button
                @click="deleteMethods(scope.row, $event)"
                type="text"
                style="color: #f56c6c"
                >删除
              </el-button>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="工号" prop="userAccount">
                <el-input
                  v-model="addOrEditData.userAccount"
                  placeholder="请输入工号"
                  style="width: 200px"
                  :disabled="myDisable"
                  clearable
            ...
            <el-col :span="12">
              <el-form-item label="姓名" prop="userName">
                <el-input
                  v-model="addOrEditData.userName"
                  placeholder="请输入姓名"
                  style="width: 200px"
                  clearable
                  maxlength="32"
            ...
            <el-col :span="12">
              <el-form-item label="角色" prop="roleId">
                <el-select
                  v-model="addOrEditData.roleId"
                  placeholder="请选择角色"
                  style="width: 200px"
                  clearable
                >
                  <el-option
                    v-for="item in rolesObjectData"
                    :key="item.id"
                    :label="item.roleName"
                    :value="item.roleId * 1"
                  ></el-option>
            ...
            <el-col>
              <el-form-item label="病区/科室" prop="regionValue">
                <el-select
                  v-model="addOrEditData.regionType"
                  placeholder="病区"
                  style="width: 200px; margin-bottom: 20px"
                  @change="regionCheage"
                >
                  <el-option
                    v-for="(item, key) in regionSelectData"
                    :key="key"
...
export default {
  data() {
    var validateAccount = (rule, value, callback) => {
      if (value) {
        // 正则表达式只匹配数字，长度 1 到 100 位
        var usernameRegex = /^[0-9]{1,100}$/;
        var isUsernameValid = usernameRegex.test(value);
        if (!isUsernameValid) {
          return callback(new Error("工号只能由数字组成"));
        } else {
          callback();
        }
      } else {
  ...
  methods: {
    ...
    editMethods(row) {
      ...
      this.addOrEditVisible = true;
      // 获取 projectStatus 里所有 dictValue 组成的数组
      const validDictValues = this.projectStatus.map((item) => item.dictValue);

      // 过滤 regionValue 中的脏数据
      let validRegionValue = [];
      if (Array.isArray(row.regionValue)) {
        validRegionValue = row.regionValue.filter((value) =>
          validDictValues.includes(String(value))
        );
      }
      this.addOrEditData = {
        userId: row.userId,
        ...
        regionValue: validRegionValue,
        // userPassword: this.initPassword,
        regionType: row.regionType,
        type: row.type,
      };
      console.log("2222222222", this.addOrEditData);
    ...
    resetMethods(value) {
      console.log(value);
      this.$confirm(
        "是否确定重置该账户密码为 " + "‘" + this.initPassword + "’" + "?",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
      ...
      this.$refs["elDialogForm"].validate((valid) => {
        if (valid) {
          if (this.isFlag == 1) {
            ...
            addData.roleIds = [addData.roleId];
            ...
            editData.roleIds = [editData.roleId];
